...
public class ProductController {
    ...
    public String showCheckoutInfo(Model model){
        OrderProductResponse response = productService.getOrderProductInfo(orderId);
        double totalprice = response.products()
                .stream()
                .map(p->p.price()*p.quantity())
                .mapToDouble(Double::doubleValue)
                .sum();
        double deliveryCharge = response.totalAmount() - totalprice;
         model.addAttribute("orderId",orderId);
         model.addAttribute("deliveryCharge",String.valueOf(deliveryCharge).formatted("%.2f"));
         model.addAttribute("orderProduct",response);
         model.addAttribute("total",response.totalAmount());
